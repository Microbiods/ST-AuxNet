...
def set_out_features(model, outputs,outputs_aux):
    ...
    if (isinstance(model, torchvision.models.AlexNet) or
        ...
        model.fc1 = torch.nn.Linear(inputs, outputs, bias=True)
        model.fc1.weight.data.zero_()
        model.fc1.bias.data.zero_()

        model.fc2 = torch.nn.Linear(inputs, outputs_aux, bias=True)
