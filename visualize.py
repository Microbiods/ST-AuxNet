...
dataset = utils.dataloader.Spatial()
...
cmap_bin = plt.get_cmap("binary")
# plt.scatter([-1], [-1], color=cmap_bin(1 - 1e-5), label="Tumor",  s=10, linewidth=0.2, edgecolors="k")
plt.scatter([-1], [-1], color=cmap_bin(1e-5),     label="Spot", s=10, linewidth=0.2, edgecolors="k")
plt.axis([0, 1, 0, 1])
plt.gca().axis("off")
plt.gca().set_aspect("equal")
plt.xticks([])
...
plt.legend(loc="center")
plt.tight_layout()
fig.savefig(args.figroot + "spot_legend.pdf")
plt.close(fig)
...
for (patient, section) in sorted(set(ps)):
    ...
    image_filename = os.path.join('data/hist2tscript/', dataset.subtype[patient], patient, "{}_{}.tif".format(patient, section))
    ...
    value= [1 for i in range(len(value))]
    fig = plt.figure(figsize=figsize)
    cmap_bin = plt.get_cmap("binary")
...
# bin/visualize.py output/densenet121_224/top_250//BC23450_cv.npz --gene FASN
# bin/visualize.py output/densenet121_224/top_250//BC23903_cv.npz --gene FASN